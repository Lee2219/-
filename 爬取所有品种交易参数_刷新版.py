    ...
    def _wait_for_data_load(self, product_code, timeout=45):
        ...
        try:
            ...
            while waited < timeout:
                ...
                # 判断是否为月均价品种（包含-F）
                is_monthly_avg = "-F" in product_code

                if is_monthly_avg:
                    # 月均价品种：检查F后缀合约（如l2609F）
                    base_code = product_code.replace("-F", "")
                    has_f_contracts = self.driver.execute_script(f"""
                        var baseCode = arguments[0];
                        var body = document.body;
                        var bodyText = body.textContent || body.innerText || '';

                        // 检查是否有F后缀的合约（如{base_code}2609F）
                        var pattern = baseCode + '\\\\d{{4}}F';
                        var regex = new RegExp(pattern, 'i');
                        return regex.test(bodyText);
                    """, base_code)

                    if has_f_contracts:
                        print(f"   ✅ 检测到{base_code}品种的F后缀合约")
                        return True
                else:
                    # 常规品种：检查对应品种的合约代码
            ...
            for tbody_idx, tbody in enumerate(tbodies):
                try:
                    ...
                    if tbody_data and len(tbody_data) > 2:
                        # 检查是否包含目标品种的合约
                        # 判断是否为月均价品种（包含-F）
                        if "-F" in product_code:
                            base_code = product_code.replace("-F", "")
                            has_target = any(
                                row and row[0] and row[0].startswith(base_code) and row[0].endswith('F')
                        ...
                        if has_target and len(tbody_data) >= 3:
                            ...
                            # 判断是否为月均价品种（包含-F）
                            if "-F" in product_code:
                                # 月均价品种：提取基础代码并匹配F后缀合约
                                base_code = product_code.replace("-F", "")
                                df = df[df['合约'].astype(str).str.startswith(base_code)]
                                # 进一步过滤：只保留以F结尾的合约
                                df = df[df['合约'].astype(str).str.endswith('F')]
                            else:
                                # 常规品种：正常过滤

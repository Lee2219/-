...
class DceZhetinCrawlerCustom:
    """大商所涨跌停板公告爬虫 - 自定义页数范围版"""
    ...
    def crawl_zhetin_announcements(self, start_page=6, end_page=15):
        """爬取指定页数范围的涨跌停板相关公告"""
        total_pages = end_page - start_page + 1

        print("=" * 60)
        print("📋 大商所涨跌停板调整公告爬虫（自定义页数范围）")
        print(f"📖 爬取页数: 第{start_page}页 到 第{end_page}页 (共{total_pages}页)")
        print("=" * 60)
        ...
        try:
            ...
            for page in range(start_page, end_page + 1):
                ...
                if page_zhetin:
                    ...
                    for ann in page_zhetin:
                        print(f"      - {ann.get('title', '')[:50]}...")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='自定义页数范围的大商所涨跌停板调整公告爬虫')
    parser.add_argument('-s', '--start', type=int, default=6, help='起始页码，默认第1页')
    parser.add_argument('-e', '--end', type=int, default=15, help='结束页码，默认第10页')
    ...
    crawler = DceZhetinCrawlerCustom(headless=args.headless)
    crawler.crawl_zhetin_announcements(start_page=args.start, end_page=args.end)

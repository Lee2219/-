...
class Dce239ZhetinCrawler:
    ...
    def _scroll_and_screenshot(self, table_element, screenshot_path, table_info, is_last_table=False):
        ...
        try:
            ...
            # 如果表格在当前视口内完全可见，直接截图
            if table_height <= viewport_height - 100:
                print(f"   📸 表格完全可见，直接截图")
            ...
            # 表格超出视口，尝试调整窗口大小
            print(f"   🔧 表格超出视口，尝试调整窗口大小")

            # 设置较大的窗口尺寸（Chrome有实际限制）
            max_window_width = 3840
            max_window_height = 10000  # 设置较大的期望值

            needed_width = min(max_window_width, max(table_width + 100, viewport_width))
            needed_height = min(max_window_height, table_height + 300)

            print(f"   🔧 尝试设置窗口大小: {needed_width}x{needed_height}px")
            ...
            # 获取实际能达到的窗口大小
            actual_viewport = self.driver.execute_script("""
                return {
            ...
            actual_width = actual_viewport.get('width', needed_width)
            actual_height = actual_viewport.get('height', needed_height)

            print(f"   🖥️  实际窗口尺寸: {actual_width}x{actual_height}px")
            ...
            rect_bottom = final_check.get('rectBottom', 0)
            actual_viewport_height = final_check.get('viewportHeight', 0)

            print(f"   🔍 表格底部位置: {rect_bottom}, 视口高度: {actual_viewport_height}")
            print(f"   🔍 表格完全可见: {is_fully_visible}")

            # 计算表格还有多少未显示
            hidden_bottom = max(0, rect_bottom - actual_viewport_height)
            print(f"   📊 表格底部隐藏部分: {hidden_bottom}px")

            if is_fully_visible:
                # 表格完全可见，直接截图
                print(f"   📸 调整后表格完全可见，直接截图")
                table_element.screenshot(screenshot_path)
                print(f"   ✅ 表格截图已保存")
                self.driver.set_window_size(1920, 1080)
                return screenshot_path
            elif hidden_bottom <= 200:
                # 只有少量隐藏，向下滚动一点后截图
                print(f"   📸 只有少量隐藏({hidden_bottom}px)，滚动后截图")
                scroll_down = max(0, hidden_bottom - 50)
                ...
                # 有较多隐藏，使用滚动拼接方法
                print(f"   📐 有较多内容隐藏({hidden_bottom}px)，使用滚动拼接")
                return self._scroll_stitch_screenshot(table_element, screenshot_path, offset_top, actual_viewport_height)

        except Exception as e:
            print(f"   ⚠️  调整窗口方法出错: {e}")
            import traceback
            traceback.print_exc()
            return self._basic_screenshot_fallback(table_element, screenshot_path, offset_top)

    def _scroll_stitch_screenshot(self, table_element, screenshot_path, offset_top, viewport_height):
        """滚动拼接截图方法 - 精确定位表格位置"""
        try:
            from PIL import Image
            import os
            ...
            table_x = table_info.get('x', 0)
            table_y = table_info.get('y', 0)
            table_width = table_info.get('width', 0)
            table_height = table_info.get('height', 0)
            current_viewport_width = table_info.get('viewportWidth', 0)
            current_viewport_height = table_info.get('viewportHeight', 0)

            print(f"   📐 表格精确位置: x={table_x}, y={table_y}, 尺寸: {table_width}x{table_height}")
            print(f"   🖥️  当前视口: {current_viewport_width}x{current_viewport_height}")

            # 检查表格是否在视口内完全可见
            if table_height <= current_viewport_height - table_y - 50:
                print(f"   📸 表格在调整后完全可见，直接截图")
                table_element.screenshot(screenshot_path)
                return screenshot_path

            # 需要滚动拼接
            overlap = 80  # 重叠像素
            effective_height = current_viewport_height - overlap - 100
            num_screenshots = max(2, int((table_height - 100) / effective_height) + 1)

            print(f"   📐 需要 {num_screenshots} 次截图拼接")
            ...
            for i in range(num_screenshots):
                ...
                if i == 0:
                    ...
                    current_scroll = scroll_top + (i * effective_height) - overlap

                self.driver.execute_script(f"window.scrollTo(0, {current_scroll});")
                time.sleep(0.5)

                # 截取整页
                temp_path = str(self.output_dir / f"temp_{i}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
                self.driver.save_screenshot(temp_path)

                with Image.open(temp_path) as img:
                    # 关键：每次都重新获取表格的精确位置
                    current_table_info = self.driver.execute_script("""
                        var table = arguments[0];
                        var rect = table.getBoundingClientRect();
                    ...
                    current_table_x = current_table_info.get('x', 0)
                    current_table_y = current_table_info.get('y', 0)
                    current_table_width = current_table_info.get('width', 0)

                    print(f"   📍 第{i+1}次 - 表格位置: x={current_table_x}, y={current_table_y}")

                    # 计算裁剪区域
                    if i == 0:
                        # 第一张：从表格顶部开始
                        crop_top = max(0, int(current_table_y))
                        if num_screenshots == 1:
                            # 只有一张，截取整个表格
                            crop_height = min(table_height, img.height - crop_top)
                        else:
                            # 有多张，截取除重叠部分
                            crop_height = min(effective_height + overlap, img.height - crop_top)
                    elif i == num_screenshots - 1:
                        # 最后一张：确保包含表格底部
                        crop_top = overlap // 2
                        remaining_height = table_height - (i * effective_height) + overlap
                        crop_height = min(remaining_height + overlap, img.height - crop_top)
                    else:
                        # 中间图片
                        crop_top = overlap // 2
                    ...
                    # 水平裁剪 - 基于当前表格位置
                    # 确保能截取到完整的表格宽度
                    crop_left = max(0, int(current_table_x) - 15)
                    crop_right = min(img.width, int(current_table_x) + current_table_width + 30)

                    print(f"   ✂️  裁剪区域: left={crop_left}, top={crop_top}, width={crop_right-crop_left}, height={crop_height}")
                    ...
                    print(f"   📸 第{i+1}张截图: {cropped.width}x{cropped.height} 像素")
            ...
            if screenshots:
                ...
                print(f"   📐 拼接 {len(screenshots)} 张，最终尺寸: {max_width}x{total_height}")
                ...
                for img in screenshots:
                    x_pos = (max_width - img.width) // 2  # 水平居中
                    final_image.paste(img, (x_pos, y_pos))
                    y_pos += img.height

                # 保存
                final_image.save(screenshot_path, quality=95)
                print(f"   ✅ 完整表格截图已保存")
    ...
    def _basic_screenshot_fallback(self, table_element, screenshot_path, offset_top):
        """基本截图备用方法"""
        try:
            print(f"   📸 使用基本截图方法")
            self._scroll_to_table_safely(table_element, offset_top)
